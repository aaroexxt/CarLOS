...
sudo npm install --prefix $cwd request browserify watchify async debug child-process brain.js window-size single-line-log node-fetch finalhandler express serve-favicon play-sound progress-stream remote-file-size colors node-soundcloud;
